...
class StackedListView extends StatefulWidget {
  ...
  })  : assert(fadeOutFrom >= 0 && fadeOutFrom <= 1, 'The range of "fadeOutFrom" must be 0.0 ~ 1.0'),
        assert(widthFactor >= 0 && widthFactor <= 1, 'The range of "widthFactor" must be 0.0 ~ 1.0'),
        assert(heightFactor >= 0 && heightFactor <= 1, 'The range of "heightFactor" must be 0.0 ~ 1.0'),
        assert(widthFactor == 1 || heightFactor == 1, 'One of "widthFactor" or "heightFactor" must equal 1'),
        super(key: key);
...
class StackedListViewState extends State<StackedListView> {
  ...
  Widget build(BuildContext context) {
    final realExtent = widget.itemExtent * (widget.scrollDirection == Axis.horizontal ? widget.widthFactor : widget.heightFactor);
    return Stack(children: [
      ListView.builder(
          ...
          itemBuilder: (context, index) {
            double scroll = ((controller.offset - index * realExtent) / realExtent);
            ...
            if (currentScroll > 0 || (index == 0 && controller.offset <= 0)) {
              ...
              if (currentScroll >= widget.fadeOutFrom) {
                if (widget.fadeOutFrom == 1) {
                  ...
                  final fadeOut = ((1 - currentScroll) / (1.0 - widget.fadeOutFrom)).clamp(0.0, 1.0);
            ...
            return AnimatedItemWidget(
              ...
              child: SizedBox(
                width: widget.scrollDirection == Axis.vertical ? double.infinity : widget.itemExtent,
                height: widget.scrollDirection == Axis.vertical ? widget.itemExtent : double.infinity,
...
class _AnimatedItemWidget extends State<AnimatedItemWidget> with TickerProviderStateMixin {
  ...
  _animateTo(double end) {
    animationController
      ..stop()
      ..reset();
    animation = Tween<double>(begin: _dragOffset, end: end).animate(animationController);
  ...
  _dragUpdate(DragUpdateDetails details) {
    _dragOffset += widget.scrollDirection == Axis.vertical ? details.delta.dx : details.delta.dy;
